...
export default function PaymentsPage() {
  ...
  const pending = jobs.filter(j => j.payment_status === 'Unpaid' && (j.status as any) === 'Completed')
  const cashPending = jobs.filter(j => j.payment_status === 'Cash Pending Verification')
  const linkSent = jobs.filter(j => j.payment_status === 'Payment Link Sent')

  const totalCollected = jobs.reduce((s, j) => s + (j.amount_collected || 0), 0)
  const totalOutstanding = jobs
    .filter(j => !['Cancelled', 'No Show'].includes(j.status))
  ...
  jobs.forEach(j => {
    if (j.payment_method && (j.amount_collected || 0) > 0) {
      if (!byMethod[j.payment_method]) byMethod[j.payment_method] = { amount: 0, count: 0 }
      byMethod[j.payment_method].amount += j.amount_collected || 0
      byMethod[j.payment_method].count++
    }
  })
  const methodData = Object.entries(byMethod).map(([name, d]) => ({ name, ...d })).sort((a, b) => b.amount - a.amount)

  const recentPaid = [...jobs]
    .filter(j => (j.amount_collected || 0) > 0)
    .sort((a, b) => new Date(b.updated_at ?? '').getTime() - new Date(a.updated_at ?? '').getTime())
    .slice(0, 15)

  const linksSent = jobs.filter(j => j.payment_link_sent).length
  const linksConverted = jobs.filter(j => j.payment_link_sent && j.payment_status === 'Paid').length
  const linksPending = jobs.filter(j => j.payment_link_sent && j.payment_status === 'Payment Link Sent').length
  const linkConversion = linksSent ? (linksConverted / linksSent) * 100 : 0
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard title="Cash Pending Verification" value={cashPending.reduce((s, j) => s + (j.amount_collected || 0), 0)} format="currency" warning />
          <StatCard title="Payment Link Conversion" value={linkConversion} format="percent" />
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-2 bg-gray-900 border border-gray-800 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {[
                    { label: 'Paid', count: paid.length, amount: paid.reduce((s, j) => s + (j.amount_collected || 0), 0), color: 'emerald' },
                    { label: 'Cash Pending', count: cashPending.length, amount: cashPending.reduce((s, j) => s + (j.amount_collected || 0), 0), color: 'amber' },
                    { label: 'Link Sent', count: linkSent.length, amount: linkSent.reduce((s, j) => s + (j.final_price || j.estimated_price || 0), 0), color: 'blue' },
                    { label: 'Unpaid', count: pending.length, amount: pending.reduce((s, j) => s + (j.final_price || j.estimated_price || 0), 0), color: 'red' },
                  ].map(stat => (
                ...
                <div className="border-t border-gray-800 pt-3">
                  ...
                  <div className="flex items-center gap-6">
                    <div><div className="text-xs text-gray-500">Links Sent</div><div className="text-lg font-semibold text-white">{linksSent}</div></div>
                    <div><div className="text-xs text-gray-500">Converted</div><div className="text-lg font-semibold text-emerald-400">{linksConverted}</div></div>
                    <div><div className="text-xs text-gray-500">Pending</div><div className="text-lg font-semibold text-amber-400">{linksPending}</div></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-lg">
              ...
              {recentPaid.length === 0 ? (
                <div className="text-center py-12 text-gray-600 text-sm">No payments yet</div>
              ) : (
                  ...
                  <tbody>
                    {recentPaid.map(job => (
                      <tr key={job.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-white font-medium">{job.customer?.name}</td>
                        <td className="px-4 py-3 text-gray-400">{job.service_type}</td>
                        <td className="px-4 py-3 text-gray-300">{job.payment_method || '—'}</td>
                        <td className="px-4 py-3 text-white font-semibold tabular-nums">{formatCurrency(job.amount_collected ?? 0)}</td>
