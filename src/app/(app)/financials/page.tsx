...
const cashCollected = DEMO_JOBS.filter(j => j.payment_method === 'Cash').reduce((s, j) => s + j.amount_collected, 0)
const cardCollected = DEMO_JOBS.filter(j => j.payment_method === 'Card').reduce((s, j) => s + j.amount_collected, 0)
const linkCollected = DEMO_JOBS.filter(j => j.payment_method === 'Payment Link').reduce((s, j) => s + j.amount_collected, 0)
const otherCollected = DEMO_JOBS.filter(j => !['Cash', 'Card', 'Payment Link'].includes(j.payment_method || '')).reduce((s, j) => s + j.amount_collected, 0)

// Monthly revenue trend (last 4 weeks by week)
const now = new Date()
const weeklyData = Array.from({ length: 4 }, (_, i) => {
  ...
  const revenue = jobs.reduce((s, j) => s + (j.final_price || 0), 0)
  return {
    week: `W${4 - i}`,
    revenue,
    jobs: jobs.length,
    avgTicket: jobs.length ? revenue / jobs.length : 0,
  }
}).reverse()
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-gray-800 border border-gray-700 rounded px-3 py-2 text-xs">
        ...
        {payload.map(p => (
          <p key={p.name} className="text-white">{p.name}: {typeof p.value === 'number' && p.name !== 'jobs' ? formatCurrency(p.value) : p.value}</p>
        ))}
      </div>
    )
  }
