...
import { DEMO_TECHNICIANS, DEMO_JOBS } from '@/lib/demo-data'
import { formatCurrency } from '@/lib/utils'
import { Phone, Mail, TrendingUp, Briefcase, DollarSign, AlertCircle } from 'lucide-react'

export default function TechniciansPage() {
  const techStats = DEMO_TECHNICIANS.map(tech => {
    const jobs = DEMO_JOBS.filter(j => j.technician_id === tech.id)
    ...
    const cashPending = jobs.filter(j => j.cash_verification_status === 'pending').reduce((s, j) => s + (j.amount_collected || 0), 0)
    const avgTicket = completed.length ? revenue / completed.length : 0
    const activeJobs = jobs.filter(j => ['Scheduled', 'Assigned', 'En Route', 'In Progress', 'Arrived'].includes(j.status))
    return { ...tech, jobCount: jobs.length, completedCount: completed.length, revenue, avgTicket, cashCollected, cashPending, activeJobs: activeJobs.length }
  })
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <Header
        title="Technicians"
        subtitle={`${DEMO_TECHNICIANS.length} technicians`}
        actions={
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${
                    i === 0 ? 'bg-amber-500 text-gray-950' : 'bg-gray-800 text-gray-300'
                  }`}>
